...
let view' model dispatch =
    installBeforeUnloadWarning model
...
let private isShortcutKey (key: string) =
...
let keyPressListener initial =
    let subDown dispatch =
        Browser.Dom.document.addEventListener("keydown", fun e ->
            ...
            let target: obj = ke?target
            let key = string ke?key
            let canvasShortcuts = shouldHandleCanvasShortcuts target

            if canvasShortcuts then
                dispatch <| Sheet(SheetT.ManualKeyDown key)

            if (jsToBool ke.ctrlKey || jsToBool ke.metaKey) && firstPress && canvasShortcuts then
                firstPress <- false
    ...
    let subUp dispatch =
        Browser.Dom.document.addEventListener("keyup", fun e ->
            let ke: KeyboardEvent = downcast e
            let target: obj = ke?target
            let key = (string ke?key).ToUpper()

            if shouldHandleCanvasShortcuts target then
                dispatch <| Sheet(SheetT.ManualKeyUp (string ke?key))

            if key = "CONTROL" || key = "META" then
                firstPress <- true
    ...
    let subContextMenuCommand dispatch =
        renderer.ipcRenderer.on("context-menu-command", fun ev args ->
            let arg = contextMenuArgsToString (unbox args)
            match arg.Split [|','|] |> Array.toList with
            | [ menuType ; item ] ->
                dispatch <| ContextMenuItemClick(menuType,item)
            | _ -> printfn "Unexpected callback argument sent from main.") |> ignore

    let subBrowserContextMenuCommand dispatch =
        addBrowserContextMenuCommandListener (fun ev ->
            let arg = contextMenuArgsToString (getCustomEventDetail ev)
            match arg.Split [|','|] |> Array.toList with
            | [ menuType ; item ] ->
                dispatch <| ContextMenuItemClick(menuType,item)
            | _ -> printfn "Unexpected browser callback argument sent from menu.")
    ...
    Cmd.batch [
        ...
        Cmd.ofSub subBrowserContextMenuCommand
