...
let processContextMenuClick
    ...
    let withSub (sub: (Msg -> unit) -> unit) (model: Model) = model, Cmd.ofSub sub
    let withSheetMsg (sheetMsg: SheetT.Msg) (model: Model) = model, Cmd.ofMsg (Sheet sheetMsg)
    let withSheetMsgs (sheetMsgs: SheetT.Msg list) (model: Model) =
        model, Cmd.batch (sheetMsgs |> List.map (Sheet >> Cmd.ofMsg))
    ...
    | SheetMenuBreadcrumb(sheet,_), "Rename" ->
        ...
        | Some project ->
            model
            |> withSub (fun dispatch -> renameFileInProject sheet.SheetName project model dispatch)
        | None ->
            withNoCmd model
    | SheetMenuBreadcrumb(sheet,_), "Delete" ->
        model
        |> withSub (fun dispatch -> deleteFileConfirmationPopup sheet.SheetName model dispatch)
    ...
    | DBCustomComp(_,ct), "Go to sheet" ->
        ...
        |> withSub (fun dispatch -> openFileInProject ct.Name p model dispatch)

    | DBComp sym, "Rotate Clockwise (Ctrl+Right)" ->
        model
        |> set (sheet_ >-> SheetT.selectedComponents_) [sym.Id]
        |> withSheetMsg (SheetT.Rotate Degree90)

    | DBComp sym, "Rotate AntiClockwise (Ctrl+Left)" ->
        model
        |> set (sheet_ >-> SheetT.selectedComponents_) [sym.Id]
        |> withSheetMsg (SheetT.Rotate Degree270)
    
    | DBComp sym, "Flip Vertical (Ctrl+Up)" ->
        model
        |> set (sheet_ >-> SheetT.selectedComponents_) [sym.Id]
        |> withSheetMsg (SheetT.Flip SymbolT.FlipVertical)
    
     | DBComp sym, "Flip Horizontal (Ctrl+Down)" ->
        model
        |> set (sheet_ >-> SheetT.selectedComponents_) [sym.Id]
        |> withSheetMsg (SheetT.Flip SymbolT.FlipHorizontal)
    ...
    | DBComp _, "Delete (DEL)" ->
        model  
        |> withSheetMsg (SheetT.KeyPress SheetT.KeyboardMsg.DEL)
    ...
    | DBScalingBox selectedcomps, "Rotate Clockwise (Ctrl+Right)"->
        model 
        |> withSheetMsgs [
            SheetT.Rotate Degree90
            SheetT.Msg.Wire (BusWireT.Msg.UpdateConnectedWires selectedcomps)
        ]

    | DBScalingBox selectedcomps, "Rotate AntiClockwise (Ctrl+Left)"->
        model 
        |> withSheetMsgs [
            SheetT.Rotate Degree270
            SheetT.Msg.Wire (BusWireT.Msg.UpdateConnectedWires selectedcomps)
        ]
    ...
    | DBScalingBox selectedcomps, "Flip Vertical (Ctrl+Up)"->
        model 
        |> withSheetMsgs [
            SheetT.Flip SymbolT.FlipVertical
            SheetT.Msg.Wire (BusWireT.Msg.UpdateConnectedWires selectedcomps)
        ]
    ...
    | DBScalingBox selectedcomps, "Flip Horizontal (Ctrl+Down)" ->
        model 
        |> withSheetMsgs [
            SheetT.Flip SymbolT.FlipHorizontal
            SheetT.Msg.Wire (BusWireT.Msg.UpdateConnectedWires selectedcomps)
        ]
    ...
    | DBScalingBox _, "Copy Box (Ctrl+C)" ->
        model  
        |> withSheetMsg (SheetT.KeyPress SheetT.KeyboardMsg.CtrlC)
    ...
    | DBCanvas pos, "Zoom-out (Alt+Down)" ->
        model
        |> withSheetMsg (SheetT.KeyPress SheetT.KeyboardMsg.ZoomOut)

    | DBCanvas _, "Fit to window (Ctrl+W)" ->
        model
        |> withSheetMsg (SheetT.KeyPress SheetT.KeyboardMsg.CtrlW)
    
    | DBCanvas pos, "Paste (Ctrl+V)" ->
        model
        |> withSheetMsg (SheetT.KeyPress SheetT.KeyboardMsg.CtrlV)
    ...
    | WaveSimHelp, feature ->
        model
        |> withSub (fun dispatch -> UIPopups.viewWaveInfoPopup dispatch feature)
