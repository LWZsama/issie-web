...
let private shouldPersistSessionSnapshot (msg: Msg) =
    match msg with
    | AnyKeyPress _
    | DiagramMouseEvent
    | DoNothing
    ...
    | Sheet sheetMsg ->
        match sheetMsg with
        | SheetT.MouseMsgOrig (_, op, _) -> op <> DrawHelpers.Move && op <> DrawHelpers.Drag
        | SheetT.UpdateScrollPos _
        | SheetT.UpdateScrollPosFromCanvas _
        | SheetT.UpdateBoundingBoxes
        | SheetT.UpdateSingleBoundingBox _
        | SheetT.ManualKeyDown _
        | SheetT.ManualKeyUp _
        | SheetT.CheckAutomaticScrolling
        | SheetT.DoNothing
        | SheetT.SetSpinner _
        | SheetT.PortMovementStart
        | SheetT.PortMovementEnd
        | SheetT.ToggleNet _ -> false
        | _ -> true
    | _ -> true
...
let update (msg : Msg) oldModel =
    ...
    | ContextMenuItemClick(menuType, item) ->
        processContextMenuClick menuType item model
    ...
    |> map fst_ (fun model' ->
        let updatedModel = resetDialogIfSelectionHasChanged model' oldModel
        if shouldPersistSessionSnapshot msg then
            persistSessionSnapshot updatedModel
        updatedModel)
